...
7. **`basemap`(D22)を`mcp/`・`openweb/`からも組み立てられるようにするか** — 今回はspiccato本体(`#q=`・`buildStyle`)のみ実装、`mcp/src/linkBuilder.ts`の`BuildLinkParams`・`openweb/main.ts`のリンク構築には`basemap`を渡す導線を追加していない。日本国外を扱うケースがMCPスタイル・オープンウェブスタイル経由でも増えてくれば着手候補
