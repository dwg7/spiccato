...
- **Map Intentへの明示的な`basemap`フィールド追加(D22、2026-08-21実装)**: `buildStyle()`が常に無条件で`bvmap`(日本限定データ)を背景にマージしており、日本国外を対象とするMap Intentでは使い物にならない背景しか出せない問題への対応。「bboxが国外ならCartographer側でジオメトリ判定して切り替える」案はユーザーが却下し、代わりに「Staffが問い合わせ内容から国内/海外を既に把握している」という理由でMap Intent自体に`basemap?: StyleRef`(既存の`StyleRef`型を再利用)を追加する設計にした。`basemap`未指定時は`base-style.json`のまま(ネットワーク依存ゼロを維持)、指定時は`fetchStyle`で取得した公開スタイルの`sources`/`layers`/`glyphs`/`sprite`/`terrain`を丸ごと使う(`before`/`after`へのマージ不要)。`#q=`にも`basemap=<style_id>[|label]`(単一値、`rstyle`/`ostyle`と同じ書式)を追加。実装中に「背景地図(bvmap)を表示」チェックボックス(D9)が`basemap`使用時にも表示され実質no-opになる副次的な問題を見つけ、`basemap`指定時は非表示にするよう合わせて修正した。テスト実在basemap: `stars-optgeo`カタログの`openstreetmap_jp_planet`(全世界カバー)。本番相当ビルドで、既存の(`basemap`無し)リンクが引き続き`bvmap`のまま動作すること(リグレッション無し)と、パリのbboxで`basemap=openstreetmap_jp_planet`が正しく背景を差し替えることの両方を実機確認済み(詳細はDECISIONS.md D22)。`mcp/`・`openweb/`から`basemap`を組み立てる導線は今回のスコープ外(未着手・フォローアップ参照)。**2026-08-28追記**: `dwg7/chukei`からの相談を機に、実際に使えるbasemapを2件用意した — `bvmap-dark`(base-style.jsonのbvmap関連レイヤーからterrain/hillshade/Mapterhornだけを除いた、日本向けの完全な背景。`stars.optgeo.org`のMartinに新規登録、SSH経由で配置)と`positron`(日本国外向け、既にstars側に用意されていたCARTO Positron風スタイルを採用)。`GENNAI_PROMPT.md`(`scripts/build-gennai-prompt.mjs`)に「日本国内は`basemap`省略、国外は`basemap=positron`」という使い分けを明記した。副産物として、`stars`の`martin`が`styles.paths`を起動時1回しかスキャンしない仕様であることと、その時点でsystemdサービスではなく手動プロセスとして動いていた運用上の乱れを発見、`stars`側の別セッションに引継ぎ再起動・systemd化してもらった(詳細はDECISIONS.md D22の2026-08-28追記)。**さらに2026-08-30追記(D23)**: ユーザーが実際に組み立てた`rstyle=positron`リンクが`duplicate layer id "background"`でMapLibreエラーになる不具合を発見。原因は`positron`(basemap用スタイル)を`rstyle`(主題オーバーレイ、bvmap背景の上に重ねる)経由で使ったことによるレイヤーid衝突 — `rstyle`ではなく`basemap`を使うのが正しい。合わせて、`basemap`単体(`req`/`opt`/`rstyle`/`ostyle`すべて無し)では`#q=`が成立しない制約が現実の使われ方に合っていなかったため、ガード条件を緩和し、goal自動合成にもbasemap単体時のフォールバックを追加した(詳細はDECISIONS.md D23)。**さらに2026-08-30追記(D24)**: `basemap=positron`使用時、不透明な主題レイヤー(空中写真等)が`positron`の注記(地名等のラベル)ごと完全に覆い隠してしまう、既定のbvmapパス(`before`/`after`分割で道路・注記が主題レイヤーの上に残る)と挙動が違う、という指摘を受けた。`resolvedBasemap.style.layers`を`type === 'symbol'`(ラベル)で分割し、非symbolは主題レイヤーの前、symbolは後ろに来るよう変更(道路線自体の分離は、任意スタイルに対する汎用的な検出規約が無いため見送り)。あわせて「背景地図(bvmap)を表示」チェックボックスを`basemap`使用時も表示するようにし(D22で一旦非表示にしていた判断を撤回)、「背景地図(positron)を表示」のように動的なラベルで、`basemapLayerIds`(before/after両方)を一括トグルできるようにした(詳細はDECISIONS.md D24)。
...
7. **`basemap`(D22)を`mcp/`・`openweb/`からも組み立てられるようにするか** — 今回はspiccato本体(`#q=`・`buildStyle`)のみ実装、`mcp/src/linkBuilder.ts`の`BuildLinkParams`・`openweb/main.ts`のリンク構築には`basemap`を渡す導線を追加していない。日本国外を扱うケースがMCPスタイル・オープンウェブスタイル経由でも増えてくれば着手候補。実在basemapは`bvmap-dark`(日本向け)・`positron`(日本国外向け)の2件が既に`stars-optgeo`カタログで使える(2026-08-28)
