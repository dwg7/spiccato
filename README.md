...
A form for pasting a Map Intent by hand is still available as a fallback (e.g. for testing a Staff prompt manually, or when neither link format is buildable) — see the site itself. That form's "1. Prompt your AI" step offers two copy-pastable Staff prompts: the normal [`hfu/layers-martin` STAFF_PROMPT.md](https://github.com/hfu/layers-martin/blob/main/STAFF_PROMPT.md) (needs the AI to fetch the catalog itself), and this repo's own [GENNAI_PROMPT.md](GENNAI_PROMPT.md) (for AI that can save a system prompt but has no internet access, e.g. 源内 — generated at build time by `scripts/build-gennai-prompt.mjs`, which embeds the full layers-martin + stars-optgeo catalogs directly so the AI never has to guess a `source_id`. See [DECISIONS.md](DECISIONS.md) D10/D12).
