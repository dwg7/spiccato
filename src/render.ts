...
export function renderFormView(
  ...
  // GENNAI_PROMPT.md (this repo, D12; generated by
  // scripts/build-gennai-prompt.mjs) has no ````text```` (quadruple-backtick)
  // fence to extract -- the whole file *is* the prompt, unlike
  // STAFF_PROMPT.md's meta-commentary-plus-fenced-block structure. Its own
  // triple-backtick fences (wrapping the embedded catalog listing) don't
  // match extractStaffPromptBlock's quadruple-backtick pattern, so that
  // function's existing fallback (return the trimmed whole markdown when no
  // fence matches) is reused as-is here rather than writing a second
  // function.
  ...
  <div class="wrap">
    ...
    <div class="card">
      ...
      <details class="dads-disclosure" style="margin-top:.5rem;">
        ...
        <div class="dads-disclosure__content">
          <p style="font-size:0.82rem;color:#555;">If your AI can save a system prompt but can't fetch the catalog itself, use this self-contained variant instead -- it embeds the full layers-martin + stars-optgeo catalogs directly, so your AI never has to guess a source_id. Prompt source: <a href="https://github.com/dwg7/spiccato/blob/main/GENNAI_PROMPT.md" target="_blank" rel="noreferrer">dwg7/spiccato GENNAI_PROMPT.md</a></p>
